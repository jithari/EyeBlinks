...
#"adjust_gamma" - a translation between the sensitivity of our eyes and sensors of a camera(controls the overall brightness of an image).
...
def blink_detector(output_textfile,input_video):
    ...
    def STUDENTSLEEPING(ear, COUNTER):
        if ear < 0.21:
            COUNTER += 1

            if COUNTER >= 50:
                print('STUDENTSLEEPING SITUATION (EYES TOO LONG CLOSED)')
                print(COUNTER)
                COUNTER = 0
        else:
            COUNTER = 0
    ...
    while True:
        ...
        # Brighten the image(Gamma correction)
        ...
        if (np.size(rects) != 0):
            ...
            First_frame = False
            old_gray = gray.copy()
            # determine the facial landmarks, then convert the facial landmark (x, y)-coordinates to a NumPy array
            ...
            # extract the left and right eye coordinates to compute the eye aspect ratio for both eyes
            ...
            # compute the convex hull for eyes and visualize
            leftEyeHull = cv2.convexHull(leftEye)
            rightEyeHull = cv2.convexHull(rightEye)
            cv2.drawContours(frame, [leftEyeHull], -1, (0, 255, 0), 1)
            cv2.drawContours(frame, [rightEyeHull], -1, (0, 255, 0), 1)

            ############HANDLING THE STUDENTSLEEPING SITATION################
            FRAME_COUNTER = STUDENTSLEEPING(ear,FRAME_COUNTER)
             # STUDENTSLEEPING SITUATION (EYES TOO LONG CLOSED)
...
path = 'E:\\research\DataSet\Fold1_part2\\08\\0.mp4' # the path to the input video
