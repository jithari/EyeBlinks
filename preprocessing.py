##############################
#Team - Titans
#Module - EyeBlinkModue - preprocessing.py
...
def temxy(ID,folder,window_size,stride,path1):
    print("#########\n")
    ...
    for txt_file in files_per_person:
        if txt_file == 'alert.txt':
            ...
            normalized_blinks = normalize_blinks(remained_size, Freq, u_Freq, sigma_Freq, Amp, u_Amp, sigma_Amp, Dur,
                                                 u_Dur, sigma_Dur,
                                                 Vel, u_Vel, sigma_Vel)
...
def gen(folder_list,window_size,stride,path1):
    for ID, folder in enumerate(folder_list):
        tempX, tempY = temxy(ID,folder, window_size, stride, path1)
        if ID > 0:
...
window_size = 30
stride = 2
Training = './Blinks_30_Foldx.npy'
Testing = './BlinksValidate_30_Foldx.npy'
#################Normalizing with respect to different individuals####First Phase
blinks,labels,blinksTest,labelTest=Preprocess(path1,window_size,stride,test_fold='Fold_Val')
#np.save(open(Training,'wb'),blinks)
#np.save(open('./Labels_30_Foldx.npy', 'wb'),labels)
#np.save(open(Testing, 'wb'),blinksTest)
#np.save(open('./LabelsTest_30_Foldx.npy', 'wb'),labelTest)
np.save(open(Testing, 'wb'),blinksTest)
np.save(open('./LabelsVal_30_Foldx.npy', 'wb'),labelTest)
